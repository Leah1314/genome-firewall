(function attachGenomeFirewallEngine(global) {
  ...
  // E. coli K-12 MG1655 reference proteins (NCBI RefSeq GCF_000005845.2),
  // not guessed -- an earlier motif set for gyrA/parC was largely fabricated
  // and failed against real sequence. gyrA/parC motifs sit well clear of the
  // QRDR (~residues 67-106), the fluoroquinolone-resistance mutation
  // hotspot, so a resistant strain's point mutation there doesn't make its
  // own target-presence check fail.
  ...
  // Real genes translate as one uninterrupted run with no in-frame stop
  // codon. Splitting on "*" and requiring every matched motif to fall within
  // the same segment means a hit reflects one candidate coding region, not
  // an arbitrary combination of motifs scattered anywhere in the genome.
